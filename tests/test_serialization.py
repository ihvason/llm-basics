...
class _TestNet(nn.Module):
    """Small two-layer network used by the checkpoint round-trip test."""
    ...
    def forward(self, x):
        """Run the two layers with a ReLU in between."""
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        x = self.fc3(x)
        return x
...
def are_optimizers_equal(optimizer1_state_dict, optimizer2_state_dict, atol=1e-8, rtol=1e-5):
    ...
    """Check that two optimizers hold identical state."""
...
def test_checkpointing(tmp_path):
    """Check that a saved checkpoint restores the model, optimizer and step."""
