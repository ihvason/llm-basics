...
# Snapshot directory, resolved relative to this file rather than to the current
# working directory, so the same snapshots are found whether pytest runs from
# the repository root or via an explicit path.

_TESTS_DIR = Path(__file__).resolve().parent
_SNAPSHOT_DIR = _TESTS_DIR / "_snapshots"


class DEFAULT:
    """Sentinel type marking an argument that was not supplied."""
    pass


def _canonicalize_array[A: (np.ndarray, Tensor)](arr: A) -> np.ndarray:
    """Convert a tensor to a NumPy array, leaving arrays untouched."""
...
class Snapshot[A: (np.ndarray, Tensor)]:
    """Pickle-backed snapshot storage for arbitrary Python objects."""
    def __init__(
        self,
        snapshot_dir: str | Path = _SNAPSHOT_DIR,
        default_force_update: bool = False,
    ...
    def _get_snapshot_path(self, test_name: str) -> Path:
        """Path of the pickle file backing a named snapshot."""
...
def ts_state_dict(request):
    """Reference GPT-2 state dict and config used by the model tests."""
    import json
...
@pytest.fixture
def n_layers():
    """Number of Transformer blocks in the reference model."""
    return 3


@pytest.fixture
def vocab_size():
    """Vocabulary size used by the test fixtures."""
    return 10_000


@pytest.fixture
def batch_size():
    """Batch size used by the test fixtures."""
    return 4


@pytest.fixture
def n_queries():
    """Number of query positions used by the test fixtures."""
    return 12


@pytest.fixture
def n_keys():
    """Number of key positions used by the test fixtures."""
    return 16


@pytest.fixture
def n_heads():
    """Number of attention heads used by the test fixtures."""
    return 4
...
def d_model(n_heads, d_head):
    """Model width, derived from the head count and head size."""
    return n_heads * d_head


@pytest.fixture
def d_ff():
    """Feed-forward hidden width used by the test fixtures."""
    return 128


@pytest.fixture
def q(batch_size, n_queries, d_model):
    """Deterministic query tensor of shape (batch_size, n_queries, d_model)."""
    torch.manual_seed(1)
...
def k(batch_size, n_keys, d_model):
    """Deterministic key tensor of shape (batch_size, n_keys, d_model)."""
    torch.manual_seed(2)
...
def v(batch_size, n_keys, d_model):
    """Deterministic value tensor of shape (batch_size, n_keys, d_model)."""
    torch.manual_seed(3)
    return torch.randn(batch_size, n_keys, d_model)
...
def in_embeddings(batch_size, n_queries, d_model):
    """Deterministic input embeddings of shape (batch_size, n_queries, d_model)."""
    torch.manual_seed(4)
    return torch.randn(batch_size, n_queries, d_model)
...
def mask(batch_size, n_queries, n_keys):
    """Deterministic boolean mask of shape (batch_size, n_queries, n_keys)."""
    torch.manual_seed(5)
...
def in_indices(batch_size, n_queries):
    """Deterministic token id tensor of shape (batch_size, n_queries)."""
    torch.manual_seed(6)
...
@pytest.fixture
def theta():
    """RoPE wavelength base used by the test fixtures."""
    return 10000.0


@pytest.fixture
def pos_ids(n_queries):
    """Token positions from 0 to n_queries - 1."""
