...
def memory_limit(max_mem):
    """Limit address space for the wrapped test."""
    def decorator(f):
        """Wrap the test with an address-space limit."""
        def wrapper(*args, **kwargs):
            """Run the test and report peak memory usage."""
...
def get_tokenizer_from_vocab_merges_path(
    ...
    """Build the reference tokenizer from the GPT-2 vocabulary and merges."""
...
def test_roundtrip_empty():
    """Check that encoding and decoding the empty string round-trips."""
...
def test_empty_matches_tiktoken():
    """Check the empty string against tiktoken."""
...
def test_roundtrip_single_character():
    """Check a single ASCII character round-trips."""
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH,
        merges_path=MERGES_PATH,
    )
...
def test_single_character_matches_tiktoken():
    """Check a single ASCII character against tiktoken."""
    reference_tokenizer = tiktoken.get_encoding("gpt2")
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH,
        merges_path=MERGES_PATH,
...
def test_roundtrip_single_unicode_character():
    """Check a single non-ASCII character round-trips."""
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH,
        merges_path=MERGES_PATH,
    )
...
def test_single_unicode_character_matches_tiktoken():
    """Check a single non-ASCII character against tiktoken."""
    reference_tokenizer = tiktoken.get_encoding("gpt2")
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH,
        merges_path=MERGES_PATH,
...
def test_roundtrip_ascii_string():
    """Check an ASCII sentence round-trips."""
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH,
        merges_path=MERGES_PATH,
    )
...
def test_ascii_string_matches_tiktoken():
    """Check an ASCII sentence against tiktoken."""
    reference_tokenizer = tiktoken.get_encoding("gpt2")
    tokenizer = get_tokenizer_from_vocab_merges_path(
...
def test_roundtrip_unicode_string():
    """Check a mixed Unicode string round-trips."""
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH,
        merges_path=MERGES_PATH,
    )
...
def test_unicode_string_matches_tiktoken():
    """Check a mixed Unicode string against tiktoken."""
    reference_tokenizer = tiktoken.get_encoding("gpt2")
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH, merges_path=MERGES_PATH, special_tokens=["<|endoftext|>"]
    )
...
def test_roundtrip_unicode_string_with_special_tokens():
    """Check a Unicode string containing special tokens round-trips."""
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH, merges_path=MERGES_PATH, special_tokens=["<|endoftext|>"]
    )
...
def test_unicode_string_with_special_tokens_matches_tiktoken():
    """Check a Unicode string containing special tokens against tiktoken."""
    reference_tokenizer = tiktoken.get_encoding("gpt2")
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH, merges_path=MERGES_PATH, special_tokens=["<|endoftext|>"]
    )
...
def test_overlapping_special_tokens():
    """Check that overlapping special tokens are matched greedily."""
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH,
        merges_path=MERGES_PATH,
...
def test_address_roundtrip():
    """Check that the address fixture round-trips."""
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH,
        merges_path=MERGES_PATH,
    )
...
def test_address_matches_tiktoken():
    """Check the address fixture against tiktoken."""
    reference_tokenizer = tiktoken.get_encoding("gpt2")
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH,
        merges_path=MERGES_PATH,
...
def test_german_roundtrip():
    """Check the German fixture round-trips."""
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH,
        merges_path=MERGES_PATH,
    )
...
def test_german_matches_tiktoken():
    """Check the German fixture against tiktoken."""
    reference_tokenizer = tiktoken.get_encoding("gpt2")
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH,
        merges_path=MERGES_PATH,
...
def test_tinystories_sample_roundtrip():
    """Check a TinyStories sample round-trips."""
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH,
        merges_path=MERGES_PATH,
    )
...
def test_tinystories_matches_tiktoken():
    """Check a TinyStories sample against tiktoken."""
    reference_tokenizer = tiktoken.get_encoding("gpt2")
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH, merges_path=MERGES_PATH, special_tokens=["<|endoftext|>"]
    )
...
def test_encode_special_token_trailing_newlines():
    """Check encoding of a special token followed by newlines."""
    reference_tokenizer = tiktoken.get_encoding("gpt2")
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH, merges_path=MERGES_PATH, special_tokens=["<|endoftext|>"]
    )
...
def test_encode_special_token_double_newline_non_whitespace():
    """Check encoding around a special token between non-whitespace text."""
    reference_tokenizer = tiktoken.get_encoding("gpt2")
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH, merges_path=MERGES_PATH, special_tokens=["<|endoftext|>"]
    )
...
def test_encode_iterable_tinystories_sample_roundtrip():
    """Check streaming encoding in text mode round-trips."""
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH,
        merges_path=MERGES_PATH,
    )
...
def test_encode_iterable_tinystories_matches_tiktoken():
    """Check streaming encoding in text mode against tiktoken."""
    reference_tokenizer = tiktoken.get_encoding("gpt2")
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH, merges_path=MERGES_PATH, special_tokens=["<|endoftext|>"]
    )
...
def test_encode_iterable_memory_usage():
    """Check that streaming encoding stays within a memory bound."""
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH,
        merges_path=MERGES_PATH,
    )
