...
from llm_basics.modeling import (
...
def run_swiglu(
    ...
    swiglu.w_gate.weight = torch.nn.Parameter(w1_weight)
    swiglu.w_up.weight = torch.nn.Parameter(w3_weight)
    swiglu.w_down.weight = torch.nn.Parameter(w2_weight)
...
def run_multihead_self_attention(
    ...
    with torch.no_grad():
        ...
        mha.out_proj.weight.copy_(o_proj_weight)
...
def run_transformer_block(
    ...
    # 1. Instantiate the TransformerBlock
    ...
    # 2. Map reference weights to block modules
    with torch.no_grad():
        block.attn_norm.weight.copy_(weights["ln1.weight"])
        ...
        block.attn.out_proj.weight.copy_(weights["attn.output_proj.weight"])

        block.ffn_norm.weight.copy_(weights["ln2.weight"])
        block.ffn.w_gate.weight.copy_(weights["ffn.w1.weight"])
        block.ffn.w_down.weight.copy_(weights["ffn.w2.weight"])
        block.ffn.w_up.weight.copy_(weights["ffn.w3.weight"])

    # 3. Instantiate RoPE and pass it to the block
    rope = RotaryPositionalEmbedding(
        theta=theta,
...
def run_transformer_lm(
    ...
    # 2. Load embeddings, per-layer weights, final norm, and lm_head
    with torch.no_grad():
        lm.token_embeddings.weight.copy_(weights["token_embeddings.weight"])
        lm.ln_final.weight.copy_(weights["ln_final.weight"])
        lm.lm_head.weight.copy_(weights["lm_head.weight"])

        for i, layer in enumerate(lm.layers):
            layer.attn_norm.weight.copy_(weights[f"layers.{i}.ln1.weight"])
            ...
            layer.attn.out_proj.weight.copy_(
                weights[f"layers.{i}.attn.output_proj.weight"]
            )

            layer.ffn_norm.weight.copy_(weights[f"layers.{i}.ln2.weight"])
            layer.ffn.w_gate.weight.copy_(weights[f"layers.{i}.ffn.w1.weight"])
            layer.ffn.w_down.weight.copy_(weights[f"layers.{i}.ffn.w2.weight"])
            layer.ffn.w_up.weight.copy_(weights[f"layers.{i}.ffn.w3.weight"])
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    clip_gradient(parameters=parameters, max_norm=max_l2_norm, eps=1e-6)
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    return BPETokenizer.train(input_path, vocab_size, special_tokens)
