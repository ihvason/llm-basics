...
def test_softmax_matches_pytorch():
    """Check the custom softmax against torch.nn.functional.softmax."""
    x = torch.tensor(
        [
...
def test_cross_entropy():
    """Check the custom cross-entropy against PyTorch's implementation."""
    inputs = torch.tensor(
        [
            [
...
def test_gradient_clipping():
    """Check that clipping bounds the global gradient norm."""
